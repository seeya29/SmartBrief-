...
class TestSummaryFlowV3(unittest.TestCase):
    """Tests for the summaryflow_v3 API module."""
    ...
    def test_example_payload(self):
        ...
        result = flow_summarize(payload)

        # Structure checks
        for key in [
            "summary_id", "user_id", "platform", "message_id",
            "summary", "type", "intent", "urgency", "entities", "generated_at"
        ]:
        ...
        self.assertEqual(result["intent"], "confirm_meeting")
        self.assertIn(result["urgency"], {"low", "medium", "high"})

        # Entity checks
        self.assertEqual(result["entities"]["person"], ["Priya"]) 
        self.assertEqual(result["entities"]["datetime"], "2025-11-21T17:00:00Z")

        # Summary phrasing (relaxed check)
        self.assertTrue(
            result["summary"].lower().startswith("user wants confirmation") and "meeting" in result["summary"].lower()
        )
